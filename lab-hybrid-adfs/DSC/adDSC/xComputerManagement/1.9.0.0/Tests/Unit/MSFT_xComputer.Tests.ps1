        ...
        Describe $Global:DSCResourceName {
            ...
            Context "$($Global:DSCResourceName)\Test-TargetResource" {
                ...
                It 'Throws if both DomainName and WorkGroupName are specified' {
                    {Test-TargetResource -Name $Env:ComputerName -DomainName 'centoso.tk' -WorkGroupName 'workgroup'} | Should Throw
                }
                It 'Throws if Domain is specified without Credentials' {
                    {Test-TargetResource -Name $Env:ComputerName -DomainName 'centoso.tk'} | Should Throw
                }
                It 'Should return True if Domain name is same as specified' {
                    Mock Get-WMIObject {[PSCustomObject]@{Domain = 'centoso.tk';Workgroup='centoso.tk';PartOfDomain=$true}}
                    Mock GetComputerDomain {'centoso.tk'}
                    Test-TargetResource -Name $Env:ComputerName -DomainName 'centoso.tk' -Credential $Credential | Should Be $true
                }
                ...
                It 'Should return True if ComputerName and Domain name is same as specified' {
                    ...
                    Mock GetComputerDomain {'centoso.tk'}
                    Test-TargetResource -Name $Env:ComputerName -DomainName 'centoso.tk' -Credential $Credential | Should Be $true
                    Test-TargetResource -Name 'localhost' -DomainName 'centoso.tk' -Credential $Credential | Should Be $true
                }
                ...
                It 'Should return False if ComputerName is not same as specified' {
                    ...
                    Mock Get-WMIObject {[PSCustomObject]@{Domain = 'centoso.tk';Workgroup='centoso.tk';PartOfDomain=$true}}
                    Mock GetComputerDomain {'centoso.tk'}
                    Test-TargetResource -Name $NotComputerName -DomainName 'centoso.tk' -Credential $Credential | Should Be $false
                }
                It 'Should return False if Computer is in Workgroup and Domain is specified' {
                    Mock Get-WMIObject {[PSCustomObject]@{Domain = 'centoso.tk';Workgroup='centoso.tk';PartOfDomain=$false}}
                    Mock GetComputerDomain {''}
                    Test-TargetResource -Name $Env:ComputerName -DomainName 'centoso.tk' -Credential $Credential | Should Be $false
                    Test-TargetResource -Name 'localhost' -DomainName 'centoso.tk' -Credential $Credential | Should Be $false
                }
            ...
            Context "$($Global:DSCResourceName)\Set-TargetResource" {
                ...
                Mock Add-Computer {}
                It 'Throws if both DomainName and WorkGroupName are specified' {
                    {Set-TargetResource -Name $Env:ComputerName -DomainName 'centoso.tk' -WorkGroupName 'workgroup'} | Should Throw
                ...
                It 'Throws if Domain is specified without Credentials' {
                    {Set-TargetResource -Name $Env:ComputerName -DomainName 'centoso.tk'} | Should Throw
                    Assert-MockCalled -CommandName Rename-Computer -Exactly 0 -Scope It
                    Assert-MockCalled -CommandName Add-Computer -Exactly 0 -Scope It
                }
                ...
                It 'Changes ComputerName and changes Workgroup to Domain' {
                    ...
                    Mock GetComputerDomain {''}
                    Set-TargetResource -Name $NotComputerName -DomainName 'centoso.tk' -Credential $Credential | Should BeNullOrEmpty
                    Assert-MockCalled -CommandName Rename-Computer -Exactly 0 -Scope It
                    Assert-MockCalled -CommandName Add-Computer -Exactly 1 -Scope It -ParameterFilter {$DomainName -and $NewName}
                    Assert-MockCalled -CommandName Add-Computer -Exactly 0 -Scope It -ParameterFilter {$WorkGroupName}
                }
                It 'Changes ComputerName and changes Workgroup to Domain with specified OU' {
                    Mock Get-WMIObject {[PSCustomObject]@{Domain = 'centoso';Workgroup='centoso';PartOfDomain=$false}}
                    Mock GetComputerDomain {''}
                    Set-TargetResource -Name $NotComputerName -DomainName 'centoso.tk' -JoinOU 'OU=Computers,DC=centoso,DC=com' -Credential $Credential | Should BeNullOrEmpty
                    Assert-MockCalled -CommandName Rename-Computer -Exactly 0 -Scope It
                    Assert-MockCalled -CommandName Add-Computer -Exactly 1 -Scope It -ParameterFilter {$DomainName -and $NewName}
                    Assert-MockCalled -CommandName Add-Computer -Exactly 0 -Scope It -ParameterFilter {$WorkGroupName}
                }
